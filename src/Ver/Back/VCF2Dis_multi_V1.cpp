...
void  VCF2DiffMatrixSNP_help()
{
	cout <<""
		...
		"\n"
		"\t\t-help                Show more help [hewm2008 v1.52m]\n"
		"\n";
}
...
int VCF2DisMain(In3str1v *paraFA04)
{
	...
	string * VetLine [Block];
	for(int i = 0; i < Block ; i++)
	{
		VetLine[i]=new string [VecSizeNum];
	}
	...
	if ( ( (paraFA04->Rand)>0)  &&  ((paraFA04->Rand)<1.0) )
	{
		...
		while(!SNP.eof())
		{
			...
			//split(line,inf," \t");
						...
						{
							iss>>VetLine[BlockReadCount][kkk];
						}
/*
			...
			for (int kkk=0 ; kkk<VecSizeNum ; kkk++ )
			{
				VetLine[BlockReadCount][kkk]=line.substr(lastPos, pos - lastPos);
