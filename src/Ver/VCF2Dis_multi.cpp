...
void  VCF2DiffMatrixSNP_help()
{
	cout <<""
		...
		"\n"
		"\t\t-help                Show more help [hewm2008 v1.56m]\n"
...
int VCF2DisMain(In3str1v *paraFA04)
{
	...
	string lineMM[Block];
	uint16_t* gt_batch = new uint16_t[Block * sampleNumber];
	bool* valid_batch = new bool[Block * sampleNumber];
	...
	if ( ( (paraFA04->Rand)>0)  &&  ((paraFA04->Rand)<1.0) )
	{
		...
		while(!SNP.eof())
		{
			...
			for (int si = 0; si < sampleNumber; si++) {
			    const char* gt = VetLine[BlockReadCount][SampleSite[si]];
			    if (gt[0] == '.') {
			        valid_batch[BlockReadCount * sampleNumber + si] = false;
			    } else {
			        valid_batch[BlockReadCount * sampleNumber + si] = true;
			        gt_batch[BlockReadCount * sampleNumber + si] = ((uint16_t)(unsigned char)gt[0] << 8) | (unsigned char)gt[2];
			    }
			}
			...
			for(int jj=0; jj<sampleNumber ; jj++)
			{
				...
				for(int kk=jj+1; kk<sampleNumber ; kk++)
				{
					for (int cc=0; cc<BlockReadCount ; cc++)
					{
						...
						if (!valid_batch[cc * sampleNumber + jj] || !valid_batch[cc * sampleNumber + kk]) continue;
						uint16_t x = gt_batch[cc * sampleNumber + jj] ^ gt_batch[cc * sampleNumber + kk];
