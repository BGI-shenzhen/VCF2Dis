...
int VCF2DisMain(In3str1v *paraFA04)
{
	...
	for ( int ii=0; ii<sampleNumber ; ii++ )
	{
		...
		if  (AA>10)
		{
			string NewName=(Vsample[SampleSite[ii]]).substr(0,10);
			int BB=AA;
			if (TFSameName.find(NewName)!=TFSameName.end())
			{
				AA=AA-10;
				NewName=(Vsample[SampleSite[ii]]).substr(AA,10);
				if (TFSameName.find(NewName)!=TFSameName.end())
				{
					NewName[9]=NewName[9]+1;
				}
			}
