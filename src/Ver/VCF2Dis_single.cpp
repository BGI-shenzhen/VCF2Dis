	...
	for ( int ii=0; ii<sampleNumber ; ii++ )
	{
		...
		if  (AA>10)
		{
			...
			for (int attempt = 0; attempt < 29 ; ++attempt)
			{
				...
				switch(type)
				{
					case 0:  NewName[siteAA]= 97 + rand() % 26; NewName[siteBB]= 97 + rand() % 26; break;
					case 1:  NewName[siteAA]= 65 + rand() % 26; NewName[siteBB]= 65 + rand() % 26; break;
					case 2:  NewName[siteAA]= 48 + rand() % 10; NewName[siteBB]= 48 + rand() % 10; break;
...
*/
	...
	uint16_t* gt_packed = new uint16_t[sampleNumber];
	bool* valid_gt = new bool[sampleNumber];
	...
	if ( ( (paraFA04->Rand)>0)  &&  ((paraFA04->Rand)<1.0) )
	{
		...
		spline.reserve(VecSizeNum);
		for (int ii=0 ; ii<VecSizeNum ; ii++)
		{
		...
		while(!SNP.eof())
		{
			...
			if (X>Y) {continue ;}
				...
				//			spline.clear();	subparser flds(line, '\t', subparser::SKIP_EMPTY);	flds.split(spline);
			split2(line,spline,VecSizeNum);

			for (int si = 0; si < sampleNumber; si++) {
				const char* gt = spline[SampleSite[si]];
				if (gt[0] == '.') {
					valid_gt[si] = false;
				} else {
					valid_gt[si] = true;
					gt_packed[si] = ((uint16_t)(unsigned char)gt[0] << 8) | (unsigned char)gt[2];
				}
			}

			for(int jj=0; jj<sampleNumber ; jj++)
			{
				if (!valid_gt[jj]) continue;
				uint16_t pj = gt_packed[jj];

				for(int kk=jj+1; kk<sampleNumber ; kk++)
				{
					if (!valid_gt[kk]) continue;
					uint16_t x = pj ^ gt_packed[kk];
					DiffData[jj][kk] += (int)((x & 0xFF) != 0) + (int)((x >> 8) != 0);
					SumData[jj][kk] += 2;
				}
			}
		}
	...
	{
		string output_PopMatfile=(paraFA04->InStr2);
        string ext =output_PopMatfile.substr(output_PopMatfile.rfind('.') ==string::npos ? output_PopMatfile.length() : output_PopMatfile.rfind('.') + 1);

        if ( ext == "mat" || ext == "Mat" || ext == "MAT" )
        {
            output_PopMatfile=(paraFA04->InStr2).substr(0,(paraFA04->InStr2).length()-4);
        }
        else if ( ext == "gz" )
        {
            output_PopMatfile=(paraFA04->InStr2).substr(0,(paraFA04->InStr2).length()-3);
        }
        output_PopMatfile=output_PopMatfile+".group.mat";

		calculateAndOutputGroupDistance(
				(paraFA04->PopInfo),
				(paraFA04->InStr2),
				output_PopMatfile,
				TFSameName);
		GetPopTreeFig_Fun(output_PopMatfile, paraFA04);
	}
